...
I implemented better support for viewing the portfolio on mobile devices by using Tachyons flex^[**Tachyons:** [Flexbox](https://tachyons.io/docs/layout/flexbox/){target="_blank"}] classes for columns, and adjusting parts of the theme to be more mobile-friendly:^[[Styling](#styling)]

::: {.iframe url="https://beck-chan.github.io" title="Portfolio in Mobile View" zoom="100%" width="50%" new-window="false"}
:::
...
With CSS I applied a fixed top navigation bar, causing anchor links to scroll to their targets under the heading hiding information. CSS `scroll-margin` on headings is unreliable because Quarto lays pages out as a CSS grid, and Quarto's own hash handler fires before the browser has finished scrolling.

`assets/anchor-offset.html`:^[**{{< fa brands github >}} beck-chan/portfolio/assets:** [anchor-offset.html](https://github.com/beck-chan/portfolio/blob/main/assets/anchor-offset.html){target="_blank"}] Waits for the native scroll to finish, then nudges the page so the target sits `80px` below the `75px` height of the header. 
...
![Heading Hidden by Header](anchor-before.png){.pics group="anchor"}
...
![Heading Offset Below Header](anchor-after.png){.pics group="anchor"}
...
I've also written a few Quarto Extensions^[**Quarto:** [Extensions](https://quarto.org/docs/extensions/){target="_blank"}] in Lua for custom functionality I found myself reusing and wanted to simplify the implementation of.

::: {.panel-tabset}

### Code Folding

Quarto's build-in option for folding code^[**Quarto:** [HTML Code Blocks](https://quarto.org/docs/output-formats/html-code.html#folding-code){target="_blank"}] only covers executable cell output. This extension allows non-executable fenced code blocks to be wrapped in a `<details>` toggle used below:^[**{{< fa brands github >}} beck-chan/portfolio/:** [`fold-code.lua`](https://github.com/beck-chan/portfolio/blob/main/_extensions/beck-chan/fold-code/fold-code.lua){target="_blank"}]

```{.lua .fold summary="view code"}
...
Embeds an iframe with a fenced div with options to include a caption under the iframe, as well as adjust zoom, sizing, and loading configurations:^[**{{< fa brands github >}} beck-chan/portfolio/:** [`iframe.lua`](https://github.com/beck-chan/portfolio/blob/main/_extensions/beck-chan/iframe/iframe.lua){target="_blank"}]

::: {.iframe url="https://beck-chan.github.io/" title="Embedded Portfolio in Iframe"}
:::

### Mobile Category Filter

When enabled, Quarto's category page filters display in the right margin on desktop,^[**Quarto:** [Document Listings](https://quarto.org/docs/websites/website-listings.html#categories){target="_blank"}] but are hidden by default on mobile. To solve this, I embedded a category filter as an inline drop-down for mobile view:^[**{{< fa brands github >}} beck-chan/portfolio/:** [`category-filter.lua`](https://github.com/beck-chan/portfolio/blob/main/_extensions/beck-chan/category-filter/category-filter.lua){target="_blank"}]

![Inline Category Filter on Mobile](category-dropdown.png){.pics}
...
- [x] Spellchecking relevant files and looking for broken links as part of PR checks.
...
The following GitHub Workflows run when a PR is opened, or when a commit is sent up to a branch connected to a PR:

::: {.panel-tabset}

#### Spellcheck

`.github/workflows/spellcheck.yaml`^[**{{< fa brands github >}} beck-chan/portfolio/.github/workflows:** [spellcheck.yaml](https://github.com/beck-chan/portfolio/blob/main/.github/workflows/spellcheck.yaml){target="_blank"}] Checks for spelling errors in relevant `*.qmd` files, then adds a comment with the results when complete:[^spellcheck]
...
`.github/workflows/link-check.yaml`^[**{{< fa brands github >}} beck-chan/portfolio/.github/workflows:** [link-check.yaml](https://github.com/beck-chan/portfolio/blob/main/.github/workflows/link-check.yaml){target="_blank"}] Generates the static output site files with the default profile (no production PostHog tracking) and checks the output for broken links and other warnings, then adds a comment with the results when complete:[^linkcheck]
...
Quarto can finish a render with a successful exit code even when links do not resolve, so the workflow treats `WARNING` and `ERROR` lines as failures.

[^linkcheck]: **Broken Link Check Comment**:

    ```{.markdown .fold summary="view example"}
    ### Render warnings found

    WARNING: Unable to resolve link target: samples/content-engineering/validmind/missing.html
    WARNING: Unable to resolve link target: #fig-not-real
    WARNING: Unable to resolve crossref: fig-missing
    ```
